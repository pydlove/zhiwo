...
public interface MembershipPlanMapper {
    ...
    @Insert("INSERT INTO tu_membership_plan(id, name, price, original_price, features_json, sort_order, is_active) " +
            "VALUES(#{id}, #{name}, #{price}, #{originalPrice}, #{featuresJson}, #{sortOrder}, #{isActive})")
    int insert(MembershipPlan plan);

    @Update("UPDATE tu_membership_plan SET name=#{name}, price=#{price}, original_price=#{originalPrice}, " +
            "features_json=#{featuresJson}, sort_order=#{sortOrder}, is_active=#{isActive} WHERE id=#{id}")
