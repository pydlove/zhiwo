...
public interface UserMapper {
    ...
    @Select("SELECT id, username, password, status, phone, email, wx_id, wx_name as wxName, nick_name as nickName, ai_limit, track_limit as trackLimit, platform_limit as platformLimit, avatar, template, expire_date as expireDate, last_login as lastLogin, remark, can_set_email as canSetEmail, email_receive as emailReceive, membership_plan_id as membershipPlanId, invite_code as inviteCode, invited_by as invitedBy, is_real as isReal, is_distributor as isDistributor, is_trial as isTrial, is_account_opened as isAccountOpened, is_deleted as isDeleted, created_at as createdAt, updated_at as updatedAt FROM tu_user WHERE invite_code = #{inviteCode} AND is_deleted = 0")
    User findByInviteCode(String inviteCode);

    @Select("SELECT COUNT(*) FROM tu_user WHERE is_deleted = 0")
    int countUsers();

    @Select("SELECT id, username, phone, email, wx_id, is_real as isReal, is_account_opened as isAccountOpened, is_distributor as isDistributor, is_trial as isTrial FROM tu_user WHERE status = 1 AND is_deleted = 0 ORDER BY created_at DESC")
