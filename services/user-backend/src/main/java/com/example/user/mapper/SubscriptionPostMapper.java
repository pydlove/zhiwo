...
public interface SubscriptionPostMapper {
    @Select("SELECT * FROM tu_subscription_post WHERE user_id = #{userId} AND track_id = #{trackId} AND is_deleted = 0 AND status = '已上架' AND DATE(created_at) = CURDATE() ORDER BY created_at DESC LIMIT 1")
